...
export const KNOWN_PROVIDERS: ProviderInfo[] = [
	...
	},
	{
		...
		requiresApiKey: true,
		oauthSupported: true,
		notes: 'Best for Claude. Direct or via OpenRouter recommended for compatibility'
	},
	{
		id: 'openai',
...
export function getProviderContract(providerId: string): ProviderRuntimeContract {
...
export function getProviderInfo(id: string): ProviderInfo | undefined {
	...
	return KNOWN_PROVIDERS.find((provider) => normalizeProviderId(provider.id) === normalized);
}
...
export function getProviderForDefaultBaseUrl(baseUrl?: string): ProviderInfo | undefined {
...
export function resolveBaseUrlForProvider(
	...
	if (defaultOwner && normalizeProviderId(defaultOwner.id) !== normalizeProviderId(providerId)) {
