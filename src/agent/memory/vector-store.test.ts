...
describe('BM25VectorStore (BM25)', () => {
	...
	it('favors shorter documents (BM25 length normalization)', async () => {
		...
		await store.addEmbedding('long', 'artificial intelligence is a fascinating field that is constantly evolving and changing our world', { doc: 2 });
	...
	it('favors higher term frequency', async () => {
		await store.addEmbedding('tf-low', 'this document mentions AI once', { doc: 1 });
		await store.addEmbedding('tf-high', 'this document mentions AI, and it talks about AI, and AI is great', { doc: 2 });
