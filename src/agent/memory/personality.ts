 ...
 */
export function extractFormattingHabitsFromDiff(diff: string): FormattingHabits {
 ...
 */
export function updateProjectProfile(projectPath: string, diff: string, commands: string[]): void {
...
export function getProjectProfile(projectPath: string): ProjectProfile | null {
	const stmt = db.prepare("SELECT * FROM project_profiles WHERE project_path = ?");
	const row = stmt.get(projectPath) as { formatting_habits: string; command_patterns: string } | undefined;
 ...
 */
export function getPersonalityPromptBlock(projectPath: string): string {
	const profile = getProjectProfile(projectPath);
	const globalGuidelines = getUserPreference("global_guidelines");
	...
	if (profile) {
		...
		if (profile.commandPatterns.frequentCommands && profile.commandPatterns.frequentCommands.length > 0) {
			block += `\n### Frequent Project Commands\n`;
			block += `- ${profile.commandPatterns.frequentCommands.join(", ")}\n`;
		}
	} else {
		block += "No specific project profile available yet. Adapt to the existing code style.\n";
	}
