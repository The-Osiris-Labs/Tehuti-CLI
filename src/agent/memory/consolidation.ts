 ...
 */
export function startBackgroundConsolidation(intervalMs: number = 60 * 60 * 1000): void {
	...
	consolidationTimer = setInterval(async () => {
		...
		try {
			...
			if (result.removed > 0 || result.merged > 0) {
				logger.debug(`Consolidated memory graph: removed ${result.removed}, merged ${result.merged} nodes.`);
			}
		} catch (error) {
			logger.error(`Error during background consolidation: ${error instanceof Error ? error.message : String(error)}`);
		} finally {
