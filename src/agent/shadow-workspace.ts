...
import type { ToolContext, ToolDefinition, ToolResult } from "./tools/registry.js";

const execAsync = promisify(exec);

const SHADOW_SCHEMA = z.object({
	command: z.string().describe("The bash command/script to execute speculatively in the shadow workspace. This should include making the changes AND running the tests."),
});
...
async function executeShadowTest(
	...
	try {
		...
		await execAsync(`git worktree add ${worktreePath} ${branchName}`, { cwd: mainDir });
		...
		if (success) {
			...
			if (status.trim()) {
				await execAsync(`git add . && git commit -m "Speculative success"`, { cwd: worktreePath });
		...
		// Cleanup
		await execAsync(`git worktree remove --force ${worktreePath}`, { cwd: mainDir }).catch(() => {});
...
export const shadowWorkspaceTool: ToolDefinition = {
	...
	description: "Create a temporary git worktree mirror, run commands (including modifying files and testing), and only apply changes back to the main workspace if the command succeeds.",
